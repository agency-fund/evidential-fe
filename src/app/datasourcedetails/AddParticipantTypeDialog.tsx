...
const AddParticipantTypeDialogInner = ({
	datasourceId,
	tables,
}: { datasourceId: string; tables: string[] }) => {
	...
	const { data: tableData, isLoading: loadingTableData } =
		useInspectTableInDatasource(datasourceId, selectedTable, {
			swr: {
	...
	useEffect(() => {
		...
		const probable_unique_id_field =
			tableData.data.detected_unique_id_fields?.pop();
...
export const AddParticipantTypeDialog = ({
	datasourceId,
}: { datasourceId: string }) => {
